...
router.patch(
  ...
  async (req, res) => {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ error: 'Erro ao aprovar usuário.', details: error.message });
    }
  }
);
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (rowCount === 0) {
        return res
          .status(404)
          .json({ error: 'Usuário não encontrado ou é o administrador.' });
      }
      return res.status(200).json({ data: rows[0] });
    } catch (error) {
      ...
      return res
        .status(500)
        .json({ error: 'Erro ao desativar usuário.', details: error.message });
    }
  }
);
