...
function CreateUserModal({ onClose, onCreated }) {
  ...
  const [form, setForm] = useState({ company_id: '', name: '', email: '', password: '', role: 'TENANT_ADMIN' });
        ...
        <form onSubmit={handleSubmit}>
          <div className="modal-body">
            ...
            <div className="form-group">
              ...
              <select className="form-input" value={form.company_id} onChange={e => set('company_id', e.target.value)} required>
                <option value="">Selecione o tenant…</option>
                {tenants.map(t => <option key={t.id} value={t.id}>{t.nome_fantasia} ({t.tenant_type})</option>)}
              </select>
            </div>
            <div className="form-group">
...
function EditUserModal({ user, onClose, onSaved }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const updates = [];
      if (form.role !== user.role) updates.push(patch(`/root/users/${user.id}/role`, { role: form.role }));
      if (form.status !== user.status) updates.push(patch(`/root/users/${user.id}/status`, { status: form.status }));
      const results = await Promise.all(updates);
      const updated = results.length > 0 ? results[results.length - 1].data : user;
      setSuccess(true);
      setTimeout(() => { onSaved({ ...user, ...updated, ...form }); }, 800);
    } catch (error) { setErr(error.message); }
    finally { setSaving(false); }
  }
    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit}>
          <div className="modal-body">
            {err && <div className="alert alert-danger" style={{ marginBottom: 14 }}>⚠ {err}</div>}
            {success && <div className="alert alert-success" style={{ marginBottom: 14 }}>✓ Usuário atualizado.</div>}

            <div style={{ padding: '10px 14px', background: 'var(--surface-3)', borderRadius: 8, marginBottom: 14, fontSize: 12, color: 'var(--text-2)' }}>
              <strong>{user.name}</strong> · {user.company_name} ({user.tenant_type})
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <div className="form-group">
                <label className="form-label">Role</label>
                <select className="form-input" value={form.role} onChange={e => set('role', e.target.value)} disabled={user.role === 'ROOT'}>
                  {(user.role === 'ROOT' ? ['ROOT'] : ROLES).map(r => <option key={r} value={r}>{r}</option>)}
                </select>
              </div>
              <div className="form-group">
                <label className="form-label">Status</label>
                <select className="form-input" value={form.status} onChange={e => set('status', e.target.value)} disabled={user.role === 'ROOT'}>
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        {success ? (
          <><div className="modal-body"><div className="alert alert-success">✓ Senha redefinida com sucesso.</div></div>
          <div className="modal-footer"><button className="btn btn-primary" onClick={onClose}>Fechar</button></div></>
        ) : (
          <form onSubmit={handleSubmit}>
            <div className="modal-body">
              {err && <div className="alert alert-danger" style={{ marginBottom: 14 }}>⚠ {err}</div>}
              <div className="form-group">
                ...
                <input className="form-input" type="password" minLength={8} placeholder="Mínimo 8 caracteres" value={password} onChange={e => setPassword(e.target.value)} required />
              </div>
              <div className="form-group" style={{ marginBottom: 0 }}>
                <label className="form-label">Confirmar senha</label>
                <input className="form-input" type="password" placeholder="Repita a senha" value={confirm} onChange={e => setConfirm(e.target.value)} required />
              </div>
            </div>
            <div className="modal-footer">
              <button type="button" className="btn btn-ghost" onClick={onClose}>Cancelar</button>
              <button type="submit" className="btn btn-primary" disabled={saving}>{saving ? 'Salvando…' : 'Redefinir'}</button>
            </div>
          </form>
        )}
      </div>
    ...
    <RootLayout title="Usuários">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <div>
          ...
          <p style={{ fontSize: 12, color: 'var(--text-3)', marginTop: 2 }}>{loading ? '…' : `${total} usuários na plataforma`}</p>
        </div>
        <button className="btn btn-primary" onClick={() => setShowCreate(true)}>+ Novo usuário gestor</button>
      </div>

      <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
        <input className="form-input" style={{ maxWidth: 260 }} placeholder="Buscar por nome ou e-mail…" value={search} onChange={e => setSearch(e.target.value)} />
        <select className="form-input" style={{ width: 170 }} value={filterRole} onChange={e => setFilterRole(e.target.value)}>
        ...
        {(search || filterRole || filterStatus) && (
          <button className="btn btn-ghost btn-sm" onClick={() => { setSearch(''); setFilterRole(''); setFilterStatus(''); }}>Limpar</button>
        )}
      </div>
      ...
      <div className="card">
        {loading ? (
          <div className="card-body">{[1,2,3,4,5].map(i => (
          ...
          <div className="empty-state">
            ...
            <div style={{ marginTop: 12 }}>
              <button className="btn btn-primary btn-sm" onClick={() => setShowCreate(true)}>Criar primeiro usuário gestor</button>
            </div>
          </div>
        ) : (
          <table className="data-table">
            <thead>
              <tr><th>Usuário</th><th>Role</th><th>Tenant</th><th>Status</th><th>Último acesso</th><th>Ações</th></tr>
            </thead>
            <tbody>
              {users.map(u => {
                  ...
                  <tr key={u.id}>
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <div style={{ width: 32, height: 32, borderRadius: '50%', background: roleStyle.bg, color: roleStyle.color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 600, flexShrink: 0 }}>
                          {getInitials(u.name)}
                        </div>
                        <div>
                    ...
                    <td><span style={{ fontSize: 11, padding: '2px 7px', borderRadius: 4, background: roleStyle.bg, color: roleStyle.color, fontWeight: 500 }}>{u.role}</span></td>
                    <td>
                    ...
                    <td style={{ fontSize: 12, color: 'var(--text-3)' }}>{formatDate(u.last_login_at)}</td>
                    <td>
                      <div style={{ display: 'flex', gap: 5 }}>
                        <button className="btn btn-ghost btn-sm" onClick={() => setEditUser(u)}>✏ Editar</button>
                        <button className="btn btn-ghost btn-sm" onClick={() => setResetUser(u)}>🔑 Senha</button>
                        {u.role !== 'ROOT' && (
                          <button className="btn btn-danger btn-sm" disabled={acting === u.id} onClick={() => deleteUser(u)}>✕</button>
                        )}
                      </div>
                    </td>
                  </tr>
      ...
      {showCreate && <CreateUserModal onClose={() => setShowCreate(false)} onCreated={(u) => { setUsers(prev => [u, ...prev]); setTotal(prev => prev + 1); setShowCreate(false); }} />}
