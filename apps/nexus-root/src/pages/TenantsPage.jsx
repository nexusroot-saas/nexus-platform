...
function ProvisionModal({ onClose, onCreated }) {
  ...
  const toggleModule = (m) =>
    setForm((p) => ({
      ...p,
      active_modules: p.active_modules.includes(m)
        ? p.active_modules.filter((x) => x !== m)
        : [...p.active_modules, m],
    }));
          ...
          <div className="modal-body">
            ...
            <div className="form-group">
              ...
              <input
                ...
                placeholder="Clínica São Lucas LTDA"
              />
            </div>
            <div
              style={{
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (form.status !== tenant.status) {
        updates.push(
          patch(`/root/tenants/${tenant.id}/status`, { status: form.status })
        );
      }
      if (
        JSON.stringify(form.active_modules.sort()) !==
        JSON.stringify((tenant.active_modules || []).sort())
      ) {
        updates.push(
          patch(`/root/tenants/${tenant.id}/modules`, {
...
export default function TenantsPage() {
  ...
  async function changeStatus(tenant, status) {
    ...
    try {
      ...
      setTenants((prev) =>
        prev.map((t) => (t.id === tenant.id ? { ...t, ...res.data } : t))
      );
    } catch (err) {
      alert(err.message);
    } finally {
      ...
      >
        <div>
          ...
          <p style={{ fontSize: 12, color: 'var(--text-3)', marginTop: 2 }}>
            {tenants.length} empresas na plataforma
          </p>
        </div>
        <button
          className="btn btn-primary"
            ...
            <tbody>
              {filtered.map((t) => (
                <tr key={t.id}>
                  ...
                  <td>
                    <span
                      className={`badge ${STATUS_BADGE[t.status] || 'badge-cancelled'}`}
                    >
                      {t.status}
                    </span>
                  </td>
