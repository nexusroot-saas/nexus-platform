  ...
  return (
    ...
    >
      ...
      {loading ? (
        <span
          className="skeleton"
          style={{ display: 'block', width: 48, height: 26, marginBottom: 4 }}
        />
      ) : (
...
export default function ManagerDashboardPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      const calls = [get('/patients'), get(`/appointments?date=${todayISO}`)];
  ...
  const STATUS_COLOR = {
    AGENDADO: 'var(--color-action-info)',
    CONFIRMADO: 'var(--color-action-success)',
    EM_ATENDIMENTO: 'var(--color-action-warning)',
    CONCLUIDO: 'var(--color-text-tertiary)',
    CANCELADO: 'var(--color-action-danger)',
    FALTOU: 'var(--color-action-danger)',
  };
  ...
  return (
    <ManagerLayout title="NexusManager">
      ...
      {pendingUsers.length > 0 && (
        ...
        >
          ⚠ {pendingUsers.length} usuário(s) aguardando aprovação — clique para
          revisar
        </div>
      )}
      {pendingConsent > 0 && (
        ...
        >
          ⚠ {pendingConsent} consentimento(s) LGPD pendente(s) — clique para
          gerenciar
        </div>
      )}
      ...
      <div className="stats-grid" style={{ marginBottom: 24 }}>
        ...
        {hasLegal && (
          <StatCard
            ...
            loading={loading}
            color={
              pendingConsent > 0 ? 'var(--color-action-warning)' : undefined
            }
            onClick={() => navigate('/consents')}
          />
        )}
