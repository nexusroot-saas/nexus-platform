              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
                {['WHATSAPP', 'EMAIL', 'TABLET'].map(ch => (
                  <label key={ch} style={{
                    ...
                    background: channel === ch ? 'var(--accent-soft)' : 'var(--surface)',
                    ...
                    <input type="radio" name="channel" value={ch} checked={channel === ch}
                      onChange={() => setChannel(ch)} style={{ display: 'none' }} />
                    <div style={{ fontSize: 18, marginBottom: 2 }}>{CHANNEL_ICON[ch]}</div>
                    <div style={{ fontWeight: channel === ch ? 500 : 400 }}>{ch}</div>
                  </label>
                ))}
              </div>
            </div>
  ...
  return (
    <AppLayout title="Consentimentos LGPD">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <div>
          <h2 style={{ fontSize: 20, fontWeight: 600, letterSpacing: '-0.3px' }}>Consentimentos</h2>
          <p style={{ fontSize: 13, color: 'var(--text-3)', marginTop: 2 }}>{total} registros</p>
        </div>
        {canCreate && (
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
        {['', 'PENDENTE', 'ASSINADO', 'RECUSADO', 'EXPIRADO', 'REVOGADO'].map(s => (
          <button
            key={s}
      ...
      <div className="card">
        ...
        {!error && !loading && consents.length === 0 && (
          <div className="empty-state">
            ...
            <div className="empty-state-title">Nenhum consentimento encontrado</div>
            <div className="empty-state-desc">
              {canCreate ? 'Envie o termo para um paciente para começar.' : 'Nenhum registro disponível.'}
            </div>
          </div>
        )}
        ...
        {!error && !loading && consents.length > 0 && (
          <table className="data-table">
            ...
            <tbody>
              {consents.map(c => (
                <tr key={c.id}>
                  <td style={{ fontWeight: 500 }}>{c.patient_name || c.patient_id.slice(0, 8) + '…'}</td>
                  <td style={{ fontFamily: 'DM Mono', fontSize: 12, color: 'var(--text-3)' }}>v{c.term_version}</td>
                  ...
                  <td style={{ fontSize: 13, color: 'var(--text-3)' }}>{formatDate(c.signed_at)}</td>
                  <td style={{ fontSize: 13, color: 'var(--text-3)' }}>{formatDate(c.created_at)}</td>
                  <td>
      ...
      {showModal && (
        <SendConsentModal onClose={() => setShowModal(false)} onSent={handleSent} />
      )}
    </AppLayout>
  );
}
