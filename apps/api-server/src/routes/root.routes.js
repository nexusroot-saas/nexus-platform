...
router.post(
  ...
  async (req, res) => {
    ...
    if (!nome_fantasia || !tenant_type) {
      return res
        .status(400)
        .json({ error: 'nome_fantasia e tenant_type são obrigatórios.' });
    }
    ...
    if (invalidModules.length > 0) {
      return res
        .status(400)
        .json({ error: `Módulos inválidos: ${invalidModules.join(', ')}` });
    }
...
router.patch(
  ...
  async (req, res) => {
    ...
    if (!Array.isArray(active_modules)) {
      return res
        .status(400)
        .json({ error: 'active_modules deve ser um array.' });
    }
    ...
    if (invalid.length > 0) {
      return res
        .status(400)
        .json({ error: `Módulos inválidos: ${invalid.join(', ')}` });
    }
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .json({ data: rows[0], message: 'Tenant cancelado com sucesso.' });
    } catch (err) {
