...
beforeAll(async () => {
  ...
  await clientA.query(
    `INSERT INTO companies (id, cnpj, nome_fantasia, tenant_type, status) 
     VALUES ($1, '11.111.111/0001-11', 'Clínica Saúde Total', 'MED', 'ACTIVE') 
  ...
  await clientA.query(
    `INSERT INTO patients (id, company_id, full_name, cpf) 
     VALUES ('00000000-0000-0000-0000-000000000101', $1, 'João Silva', '111.111.111-11') 
     ON CONFLICT DO NOTHING;`,
    [TENANT_A]
  );
  await clientA.query(
    `INSERT INTO patients (id, company_id, full_name, cpf) 
     VALUES ('00000000-0000-0000-0000-000000000102', $1, 'Maria Souza', '222.222.222-22') 
     ON CONFLICT DO NOTHING;`,
    [TENANT_A]
  );

  // Inserir pacientes Tenant B
  await clientA.query(
    `INSERT INTO patients (id, company_id, full_name, cpf) 
     VALUES ('00000000-0000-0000-0000-000000000201', $1, 'Carlos Lima', '333.333.333-33') 
     ON CONFLICT DO NOTHING;`,
    [TENANT_B]
  );
  await clientA.query(
    `INSERT INTO patients (id, company_id, full_name, cpf) 
     VALUES ('00000000-0000-0000-0000-000000000202', $1, 'Ana Costa', '444.444.444-44') 
     ON CONFLICT DO NOTHING;`,
    [TENANT_B]
  );
  ...
  await clientA.query(`
    ...
    FOR ALL 
    USING (company_id::text = current_setting('app.currentcompanyid', true))
    WITH CHECK (company_id::text = current_setting('app.currentcompanyid', true));
  `);
...
test('Tenant A pode inserir paciente no seu tenant', async () => {
  const result = await clientA.query(
    `INSERT INTO patients (id, company_id, full_name) 
     VALUES ($1, $2, $3) 
     RETURNING id;`,
