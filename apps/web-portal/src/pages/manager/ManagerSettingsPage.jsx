...
const TENANT_TYPE_LABEL = {
  MED: 'NexusMed — Clínica Médica',
  CLIN: 'NexusClin — Estética/Spa',
  ODONTO: 'NexusOdonto — Odontologia',
  LAB: 'NexusLab — Laboratório',
  IMG: 'NexusIMG — Centro de Imagem',
  ADM: 'NexusAdm — Gestão Administrativa',
};

const MODULE_LABEL = {
  NEXUSMED: 'NexusMed',
  NEXUSCLIN: 'NexusClin',
  NEXUSODONTO: 'NexusOdonto',
  NEXUSLAB: 'NexusLab',
  NEXUSIMG: 'NexusIMG',
  NEXUSLEGAL: 'NexusLegal',
  NEXUSADM: 'NexusAdm',
};
...
export default function ManagerSettingsPage() {
  ...
  function setBranding(key, value) {
    setForm((prev) => ({
      ...prev,
      config_branding: { ...prev.config_branding, [key]: value },
    }));
  }
        ...
        <div style={{ padding: 20 }}>
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="skeleton"
              style={{ height: 48, marginBottom: 16, borderRadius: 8 }}
            />
          ))}
        </div>
      </ManagerLayout>
    ...
    <ManagerLayout title="Configurações da Unidade">
      <div style={{ maxWidth: 640 }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="card" style={{ marginBottom: 16 }}>
            <div className="card-header">
              <span className="card-title">Identidade visual</span>
            </div>
            <div className="card-body">
              <div className="form-group">
                ...
                <input
                  className="form-input"
                  value={form.config_branding?.logo_url || ''}
                  onChange={(e) => setBranding('logo_url', e.target.value)}
                  placeholder="https://minha-clinica.com.br/logo.png"
                />
                <p
              ...
              <div className="form-group">
                ...
                <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                  <input
                    type="color"
                    value={form.config_branding?.primary_color || '#1a6cff'}
                    onChange={(e) =>
                  ...
                  <span style={{ fontSize: 12, color: 'var(--text-3)' }}>
                    Cor do tema da interface
                  </span>
                </div>
              </div>
              <div className="form-group" style={{ marginBottom: 0 }}>
                <label className="form-label">
                  Endereço / telefone (rodapé de documentos)
                </label>
                <input
                  className="form-input"
                  value={form.config_branding?.address || ''}
                  onChange={(e) => setBranding('address', e.target.value)}
                  placeholder="Rua das Flores, 123 — (67) 3000-0000"
                />
              </div>
            </div>
          </div>
          ...
          <div className="card" style={{ marginBottom: 24 }}>
            <div className="card-header">
              <span className="card-title">Informações da conta</span>
            </div>
            <div className="card-body">
                ...
                {[
                  {
                    ...
                    value:
                      TENANT_TYPE_LABEL[settings?.tenant_type] ||
                      settings?.tenant_type,
                  },
              ...
              <p
                style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 12 }}
              >
                Para alterar tipo de plataforma, módulos ou CNPJ, entre em
                contato com o suporte Nexus.
              </p>
            </div>
          </div>
