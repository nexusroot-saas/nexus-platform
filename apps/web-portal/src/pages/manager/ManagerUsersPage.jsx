        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="modal-footer">
            ...
            <button type="submit" className="btn btn-primary" disabled={saving}>
              {saving ? 'Criando…' : 'Criar e aguardar aprovação'}
            </button>
          </div>
        </form>
      </div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="modal-body">
            ...
            <div className="form-group" style={{ marginBottom: 0 }}>
              ...
              >
                {(user.role === 'TENANT_ADMIN' ? ['TENANT_ADMIN'] : ROLES).map(
                  (r) => (
                    <option key={r} value={r}>
                      {ROLE_LABEL[r]}
                    </option>
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-footer">
          ...
          <button
            ...
            disabled={saving}
          >
            {saving ? 'Aprovando…' : '✓ Aprovar acesso'}
          </button>
        </div>
      </div>
    </div>
...
export default function ManagerUsersPage() {
  ...
  async function toggleStatus(user) {
    ...
    if (
      !confirm(
        `${newStatus === 'BLOCKED' ? 'Bloquear' : 'Ativar'} "${user.name}"?`
      )
    )
      return;
    setActing(user.id);
    try {
      const res = await patch(`/manager/users/${user.id}`, {
        status: newStatus,
      });
      setUsers((prev) =>
  ...
  async function removeUser(user) {
    if (
      !confirm(
        `Desligar "${user.name}"? O acesso será bloqueado imediatamente.`
      )
    )
      return;
    setActing(user.id);
  ...
  return (
    <ManagerLayout title="Equipe">
      ...
      {pending.length > 0 && (
        <div
          className="card"
          style={{ marginBottom: 16, border: '1px solid var(--warning)40' }}
        >
          <div
