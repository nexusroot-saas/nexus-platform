  ...
  return (
    <AppLayout title="Trilha de Auditoria">
      <div style={{ marginBottom: 20 }}>
        <h2 style={{ fontSize: 20, fontWeight: 600, letterSpacing: '-0.3px' }}>
          Auditoria LGPD
        </h2>
      ...
      >
        ...
        <div>
          ...
          >
            <option value="">Todas</option>
            {['patients', 'appointments', 'consents', 'users', 'financial'].map(
              (t) => (
        ...
        <button
          ...
          onClick={() =>
            setFilters({ action: '', table_name: '', from: '', to: '' })
          }
        >
          Limpar
        </button>
      ...
      >
              ...
              <tbody>
                {logs.map((l) => (
                  ...
                  >
                    <td>
                      <span
                        className={`badge ${ACTION_BADGE[l.action] || 'badge-neutral'}`}
                      >
                        {l.action}
                      </span>
                    </td>
                    ...
                    <td style={{ fontSize: 13 }}>
                      {l.user_name || l.user_id?.slice(0, 8) + '…' || 'Sistema'}
                    </td>
                    <td
                      style={{
                        fontFamily: 'DM Mono',
        ...
        {selected && (
          <div
            ...
            style={{ alignSelf: 'start', position: 'sticky', top: 72 }}
          >
            ...
            <div className="card-body" style={{ fontSize: 13 }}>
              <div style={{ display: 'grid', gap: 10 }}>
                <Row
                  label="ID"
                  value={<code style={{ fontSize: 11 }}>{selected.id}</code>}
                />
                <Row
                  label="Ação"
                  value={
                    <span className={`badge ${ACTION_BADGE[selected.action]}`}>
                      {selected.action}
                    </span>
                  }
                />
                <Row
                  label="Tabela"
                  value={
                    <code style={{ fontSize: 12 }}>{selected.table_name}</code>
                  }
                />
                <Row
                  label="Registro"
