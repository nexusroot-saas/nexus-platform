  ...
  return (
    <div style={{ marginBottom: 48 }}>
      <h2 style={{
        ...
        borderBottom: '1px solid var(--color-border-default)', paddingBottom: 8,
      }}>
        {title}
      </h2>
      {children}
...
function ColorSwatch({ name, value }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4, alignItems: 'center' }}>
      <div style={{
        width: 48, height: 48, borderRadius: 8, background: value,
...
export default function Styleguide() {
  return (
    <div style={{
      ...
      fontFamily: 'var(--font-sans)', background: 'var(--color-surface-bg)', minHeight: '100vh',
    }}>
      ...
      <p style={{ color: 'var(--color-text-secondary)', marginBottom: 40, fontSize: 14 }}>
        ...
        <code style={{ fontFamily: 'var(--font-mono)', background: 'var(--color-surface-alt)', padding: '1px 6px', borderRadius: 4 }}>
          &lt;body&gt;
        </code>{' '}
      ...
      <Section title="Temas de submarca">
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          {Object.entries(brandMap).map(([key, val]) => (
            <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 12px', borderRadius: 8, background: val.soft, border: `1px solid ${val.primary}20` }}>
              <div style={{ width: 16, height: 16, borderRadius: '50%', background: val.primary }} />
              <span style={{ fontSize: 13, fontWeight: 500, color: val.primary }}>Nexus{key.charAt(0) + key.slice(1).toLowerCase()}</span>
            </div>
          ))}
        </div>
      </Section>
      ...
      <Section title="Tipografia">
        {[
          ...
          <div key={label} style={{ marginBottom: 8 }}>
            <span style={{ fontSize: size, fontWeight: weight, color: 'var(--color-text-primary)' }}>
      ...
      <Section title="Input">
        <div style={{ maxWidth: 360 }}>
          ...
          <Input label="Com erro" placeholder="campo obrigatório" error="Este campo é obrigatório." />
        </div>
      </Section>
    </div>
  );
