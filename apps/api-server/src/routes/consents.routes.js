...
router.get('/', authenticate, requireModule('NEXUSLEGAL'), authorize('consents', 'read'), async (req, res) => {
  ...
  try {
    ...
    if (patient_id) { params.push(patient_id); filters.push(`c.patient_id = $${params.length}`); }
...
router.get('/:id', authenticate, requireModule('NEXUSLEGAL'), authorize('consents', 'read'), async (req, res) => {
  const client = await pool.connect();
  try {
    const result = await client.query(
    ...
    if (result.rowCount === 0) return res.status(404).json({ error: 'Consentimento não encontrado.' });
...
router.post('/send', authenticate, requireModule('NEXUSLEGAL'), authorize('consents', 'create'), async (req, res) => {
  ...
  try {
    const result = await createConsent({
      company_id: req.user.company_id, patient_id, channel, term_version,
      created_by: req.user.sub, ip_address: req.ip,
    });
    if (result.error) return res.status(result.status || 400).json({ error: result.error });
    sendConsent(result.consent).catch((err) => console.error('[consents] sendConsent error:', err.message));
    return res.status(201).json({ data: result.consent, message: `Termo enviado via ${channel}.` });
  } catch (err) {
