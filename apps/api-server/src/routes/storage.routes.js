...
router.post(
  ...
  async (req, res) => {
    const { bucket, entity_type, entity_id, file, file_name, mime_type } =
      req.body;

    if (!bucket || !file || !file_name || !mime_type) {
      return res.status(400).json({
        error: 'bucket, file (base64), file_name e mime_type são obrigatórios.',
      });
    }

    if (!Object.values(BUCKETS).includes(bucket)) {
      return res.status(400).json({
        error: `Bucket inválido. Use: ${Object.values(BUCKETS).join(', ')}`,
      });
    }

    // Apenas TENANT_ADMIN pode fazer upload no legal-vault
    if (
      bucket === BUCKETS.LEGAL_VAULT &&
      !['TENANT_ADMIN', 'ROOT'].includes(req.user.role)
    ) {
      return res.status(403).json({
        error: 'Apenas administradores podem enviar para o legal-vault.',
      });
    }
    ...
    try {
      ...
      return res.status(200).json({
        data: {
          ...
          expires_at: new Date(
            Date.now() + doc.signed_url_expires_in * 1000
          ).toISOString(),
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .json({ message: 'Documento removido com sucesso.' });
    } catch (err) {
